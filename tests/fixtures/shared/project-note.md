---
marktodo: true
---

Notes before any heading are preserved; this line is not a todo.

## Kitchen
### Progress
- [/] Demo old floor @high <!-- mt id=a2 -->
- [/] Mixed [[A]] #x @high due @ 2026-07-01 <!-- mt id=cc -->
### Done
- [x] Order tile <!-- mt id=a5 -->
- [x] Extra key preserved @high <!-- mt id=ee foo=bar -->

## Bathroom
### Backlog
- [ ] Replace vanity <!-- mt id=a7 -->
- [ ] Plain with [[Link]] and #tag and due @ 2026-07-01
### Warming
- [>] Warm up <!-- mt id=b1 -->
### Blocked
- [!] Blocked thing @urgent <!-- mt id=b2 -->
### Paused
- [-] Paused thing @low <!-- mt id=b3 -->

## Loose ends
- plain bullet, not a todo
- [ ] Call contractor [[Contractor Notes]] notify @ 2026-03-20 09:00 delete-after @ 30d <!-- mt id=n1 -->
- [x] Synced done @ 2026-06-01 @low <!-- mt id=z9 notify=2026-09-01 src=app -->
  - [ ] Indented backlog
* [ ] Asterisk bullet
- [?] Question glyph preserved <!-- mt id=qq -->
- [ ] Email [[Carol]] re #budget before due @ 2026-08-15
